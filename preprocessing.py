...
from rtrend_tools.forecast_structs import ForecastExecutionData, ForecastOutput, AbstractNoise, CovHospForecastOutput
...
def denoise_lowpass(exd: ForecastExecutionData, fc: CovHospForecastOutput, filt_order=2, cutoff=0.4, **kwargs):
    fc.denoised_weekly = pd.Series(apply_lowpass_filter(exd.state_series.values, cutoff, filt_order),
...
def denoise_polyfit(exd: ForecastExecutionData, fc: CovHospForecastOutput, poly_degree=3, **kwargs):
    ...
    fc.denoised_weekly = pd.Series(poly_f(fc.t_daily), index=exd.state_series.index)


# noinspection PyUnusedLocal
def denoise_rolling_average(exd: ForecastExecutionData, fc: ForecastOutput, rollav_window=4, **kwargs):
    fc.denoised_weekly = exd.state_series.rolling(rollav_window).mean()  # Rolling average
    fc.denoised_weekly[:rollav_window-1] = exd.state_series[:rollav_window-1]  # Fill NAN values with original ones
...
# noinspection PyUnusedLocal
def denoise_none(exd: ForecastExecutionData, fc: ForecastOutput, **kwargs):
    fc.denoised_weekly = exd.state_series
